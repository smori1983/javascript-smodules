...
QUnit.module('template', {
  before: function () {
    this.bindAndGet = function (src, param) {
      return this.template.bind(src, param).get();
    };
  },
  beforeEach: function () {
...
QUnit.test('literal block', function (assert) {
  const src = '{literal}<p>{literal} {open}endliteral{right}</p>{endliteral}';
...
QUnit.test('holder block', function (assert) {
  ...
  assert.strictEqual(this.bindAndGet(src, param), expected);
});
